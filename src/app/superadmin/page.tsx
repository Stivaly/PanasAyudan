...
import { cerrarNodo } from "@/lib/api";
...
export default function SuperadminPanel() {
  ...
  const cerrar = async () => {
    ...
    setError(null);
    if (!nodeId.trim()) {
      setError("Ingresa el ID del nodo a cerrar.");
      return;
    }
    setEnviando(true);
    try {
      await cerrarNodo(nodeId.trim(), token);
      setMensaje("Nodo cerrado permanentemente.");
      setNodeId("");
    } catch (e) {
      setError(e instanceof Error ? e.message : "No se pudo cerrar el nodo.");
    } finally {
      setEnviando(false);
    }
  };
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-lg flex-col gap-5 p-4 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
      ...
      <div className="card border-accent flex flex-col gap-3">
        <p className="text-sm font-semibold text-accent">Cerrar nodo (permanente)</p>
        <input
          className="field"
